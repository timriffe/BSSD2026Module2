...
```{r, eval = FALSE}
library(tidyverse)
ES <- read_csv("https://raw.githubusercontent.com/timriffe/BSSD2026Module2/master/data/ESmx.csv.gz", show_col_types = FALSE)
```
...
It decomposes an $m~\times~n$ matrix in an $m~\times~m$ matrix, named $U$, (the columns consisting of left-singular vectors), an $m~\times~n$ diagonal matrix with non-negative real numbers (singular values) and an $n~\times~n$ matrix, named $V$, (the columns consisting of right-singular vectors). 
